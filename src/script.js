				...
				.then(filteredData => {
					...
					for (const category in groupedData) {
						...
						groupedData[category].forEach((item, sectionIndex) => {
							...
							// add an id to the content paragraph so we can update it later
							contentParagraph.id = `content-${sectionIndex + 1}`;
							...
							if (!item.content) {
								...
								requestButton.addEventListener('click', () => {
									...
									if (apiKey) {
										...
										content.removeChild(requestButton);
									}
								});
...
function getData(discipline, knowledge, category, section, content) {
    return fetch('src/data.json') // Fetch the JSON file
        .then(response => {
            if (!response.ok) {
...
function setData(discipline, knowledge, category, section, content) {
    return fetch('http://localhost:3000/data.json') // Fetch the JSON file from the server
        .then(response => {
            if (!response.ok) {
                throw new Error(`Failed to load data: ${response.statusText}`);
            }
        ...
        .then(data => {
            // Find the index of the object that matches the given criteria
            ...
            if (entryIndex !== -1) {
                // Update the content of the matched entry
                data[entryIndex].content = content;
            } else {
                console.log('No matching entry found.');
            }

            console.log("Sending updated data:", data); // Log the data being sent to the server

            // Send the updated data back to the server
            return fetch('http://localhost:3000/updateData', {
                method: 'POST',
                headers: {
...
async function rewriteContent({ discipline, knowledge, category, section, apiKey, index }) {
    try {
        if (!apiKey) throw new Error("API key is missing.");
        if (!discipline || !knowledge || !category || !section) {
            throw new Error("One or more required parameters are missing.");
        }

        console.log('Rewriting content...');
        const knowledgeLevels = ["Expert", "Advanced", "Intermediate", "Novice", "None"]; // Order of fallback levels
        let content = null;
        ...
        for (const level of knowledgeLevels) {
            console.log(`Trying knowledge level: ${level}`); // Debug log
        ...
        console.log(`Content found at knowledge level: ${usedLevel}`); // Log the level that worked
        ...
        if (!rewrittenContent) {
            throw new Error("Failed to generate rewritten content from OpenAI.");
        }

        // Check if the element exists before updating
        const contentElement = document.getElementById(`content-${index}`);
        console.log('Element to update:', contentElement);
        if (contentElement) {
            contentElement.textContent = rewrittenContent; // Update the content in the DOM
            console.log(`Rewritten Content (content-${index}):`, rewrittenContent);
        } else {
            console.error(`Element with id content-${index} not found.`);
        }

        // Update the server with the rewritten content
        await setData(discipline, knowledge, category, section, rewrittenContent);

        return rewrittenContent;
    } catch (error) {
        console.error("Error occurred in rewriteContent:", error.message);
